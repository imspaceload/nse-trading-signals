...
<style>
    ...
    * { font-family: 'Inter', -apple-system, sans-serif !important; }
    .block-container { padding-top: 0.5rem; max-width: 100%; padding-left: 0.5rem; padding-right: 0.5rem; }
    ...
    /* Watchlist panel (left column) */
    .wl-panel { background: #1b1b2f; border-right: 1px solid #2a2a4a; border-radius: 6px; padding: 0; min-height: 90vh; }
    .wl-header { padding: 12px 14px; border-bottom: 1px solid #2a2a4a; }
    .wl-search input { background: #12121f !important; border: 1px solid #2a2a4a !important; color: #e8e8e8 !important; border-radius: 4px !important; font-size: 13px !important; }
    .wl-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 14px; border-bottom: 1px solid #1a1a2e; cursor: pointer; transition: background 0.15s; }
    .wl-item:hover { background: rgba(99,102,241,0.1); }
    .wl-sym { color: #e8e8e8; font-size: 0.85em; font-weight: 600; }
    .wl-price { font-size: 0.85em; font-weight: 600; }
    .wl-change { font-size: 0.72em; margin-right: 6px; }
    .wl-count { color: #6b7280; font-size: 0.7em; text-transform: uppercase; letter-spacing: 1px; padding: 10px 14px 6px 14px; }

    /* Watchlist button row styling */
...
with left_col:
    ...
    st.markdown(f"""
    <div style="padding:8px 0 12px 0;border-bottom:1px solid #2a2a4a;margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="color:#e8e8e8;font-size:1.05em;font-weight:700;">Options Terminal</span>
        ...
        <div style="color:#6b7280;font-size:0.72em;margin-top:4px;">{now.strftime("%d %b %Y, %I:%M %p IST")}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="margin-top:10px;display:flex;justify-content:space-between;align-items:center;">
        <span style="color:#6b7280;font-size:0.65em;text-transform:uppercase;letter-spacing:1px;">Watchlist 1 ({len(saved_watchlist)} / 250)</span>
        <span style="color:#9ca3af;font-size:0.6em;">{len(subs)} sub <span style="color:{sms_clr};">{sms_label}</span></span>
    </div>
    """, unsafe_allow_html=True)

    # Clickable watchlist items — Kite style rows
    for wl_name in saved_watchlist:
        ...
        wl_cols = st.columns([5, 1])
        with wl_cols[0]:
            if p:
                price_str = f"{p:,.2f}"
                if st.button(f"{display_name}  ·  {price_str}", key=f"wl_{wl_name}", use_container_width=True):
                    st.session_state["_wl_selected"] = wl_name
                    st.rerun()
            else:
                if st.button(f"{display_name}  ·  --", key=f"wl_{wl_name}", use_container_width=True):
                    st.session_state["_wl_selected"] = wl_name
                    st.rerun()
        with wl_cols[1]:
    ...
    if not saved_watchlist:
        st.markdown('<div style="color:#4b5563;font-size:0.75em;padding:4px 0;">Type below to add symbols</div>', unsafe_allow_html=True)
...
with right_col:
    ...
    if selected_symbol is None:
        ...
        <div style="text-align:center;padding:80px 20px;">
            ...
            <div style="color:#6b7280;font-size:0.95em;margin-bottom:24px;">Type a symbol in search to get started</div>
...
""", height=0)
        ...
        st.warning(f"Could not fetch market data for {selected_symbol}. Will retry on next auto-refresh (60s). SMS Admin still works below.")
        action = "HOLD"


    # ══════════════════════════════════════════
    #  TABS: Chart | Option Chain | Signals | SMS Admin
    # ══════════════════════════════════════════

    tab_chart, tab_optchain, tab_signals, tab_sms = st.tabs([
        "Chart", "Option Chain", "Signals", "SMS Admin"
    ])
    ...
    # ── TAB 2: OPTION CHAIN (Dhan API → NSE fallback) ──
    with tab_optchain:
        nse_sym = sym.get("nse", "")
    ...
    with tab_signals:
        if not data_ok:
            st.error(f"Market data unavailable for {selected_symbol}. Auto-retrying every 60s. SMS Admin tab still works.")
        else:
            ...
            with action_col:
                if action == "BUY":
                    ...
                    icon_str = "HOLD"
                    steps_html = '<div class="step">No clear signal. <b>Waiting for next one...</b></div>'
                ...
                if option_rec:
                    ...
                    opt_html += f'<div class="detail">OI: {option_rec["oi"]:,} | OI Change: {option_rec["oi_change"]:,} | IV: {option_rec["iv"]}%</div>'
            ...
            with chat_col:
                ...
                if action in ("BUY", "SELL") and option_rec:
                    ...
                    chat_text = f'<b>{action} {selected_symbol} {int(option_rec["strike"])}{opt_type}</b>'
                    ...
                    chat_text = f'<b>{action} {selected_symbol} {atm_s}{opt_type}</b>'
                    ...
                    chat_text = f'<b>HOLD — {selected_symbol}</b><br>No clear signal. Scanning...'
                ...
                for s in reversed(all_signals[-2:]):
                    ...
                    s_icon = "BUY" if s["action"] == "BUY" else "SELL"
                    atm = _atm_strike(s["price"])
                    s_avg = round(s["price"] * 0.7, 2)
                    chat_html += f'<div class="chat-msg {s_css}"><b>{s["action"]} {selected_symbol} {atm}{opt_type}</b><br>Entry: ₹{s["price"]:,.2f} | T: ₹{s["target"]:,.2f} | Avg: ₹{s_avg:,.2f}<div class="chat-time">{ts}</div></div>'
                chat_html += '</div>'
            ...
            with ic5:
                st.markdown(f'<div class="indicator-card"><h4>OI / PCR</h4><div class="value">{oi_data["pcr"]}</div>{badge(oi_data["signal"])}<div style="color:#9ca3af;font-size:0.7em;margin-top:4px;">Net Change: {oi_data["net_oi_change"]:,}</div></div>', unsafe_allow_html=True)
        ...
        with sms_col1:
            ...
            if subs_list:
                ...
                for s in subs_list:
                    ...
                    status_color = "#4caf50" if s.get("active", True) else "#f44336"
                    sub_rows += f'<tr><td>{s.get("name", "-")}</td><td>{s["phone"]}</td><td style="color:{status_color};">{status}</td><td>{s.get("added", "")[:10]}</td></tr>'
    ...
    st.markdown("---")
    st.caption("For educational purposes only. Not financial advice. Always use stop loss. App auto-refreshes every 60s during market hours.")
